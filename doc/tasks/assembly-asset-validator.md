...
- [x] **AV-008 完成资产校验回归。** 对 9 个首版零件和全部目标运行校验。验证：资产校验测试及 headless 夹具测试全部通过，格式和 lint 通过。
...
- GUT `9.6.0` 单模块回归：`11/11` 测试、`76` 断言通过，覆盖有效资源、资源缺失、资源类型、根类型、脚本能力、契约节点、目标节点及重复校验清理。
- `gdformat 4.5.0 --check` 与 `gdlint 4.5.0` 对本模块全部 GDScript 检查通过。
- AV-008 使用 `ContentRepository.load_catalog()`、`res://scenes/train/train_root.tscn` 和 9 个 `res://scenes/train/parts/*.tscn` 完成真实资产回归；9 个唯一场景路径与 9 个唯一 `Marker3D` 目标全部覆盖，连续 3 次校验均无问题和场景树节点残留。
- AssemblyAssetValidator、AssemblyManager 与 AssemblyView 联合回归：`34/34` 测试、`428` 断言通过。
