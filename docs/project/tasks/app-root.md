...
- [ ] **AR-001 创建工程目录和 project.godot。** 建立详细设计第 4 节目录，设置 Godot 4.6.3、Compatibility、1280×720、最小 960×540 和窗口模式。验证：headless editor 可打开空工程。
- [ ] **AR-002 固定 Python 开发工具。** 创建 `.python-version`、`pyproject.toml`、`uv.lock`，固定 Python 3.12.13、uv 0.11.8、gdtoolkit 4.5.0、pre-commit 4.6.0。验证：`uv sync --frozen` 成功。
- [ ] **AR-003 引入固定 GUT。** 将 GUT 9.6.0 提交到 `addons/gut/` 并创建最小测试入口。验证：headless GUT 空测试集可启动并退出。
