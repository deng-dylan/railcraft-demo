...
车厢。两者都只承担视觉表现，最终展示场景继续直接使用完整 `FuxingTrain.fbx`。
...
| 车体零件/落车完成车辆 | `FuxingCarbodyAssemblyDemo.fbx` 的缩略件/1:1 车厢 + 两套完整示范转向架 |
...
- 落车工位钢轨中心距使用源模型的 `1.435 m`，车体底面与示范转向架最高点保留约 `0.045 m` 间隙；落车工位纵向扩展到约 29 m。
