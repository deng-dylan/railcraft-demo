...
本地可执行文件、测试 XML 和截图的 SHA-256 见 [`checksums.txt`](checksums.txt)。这些
文件不受 Git 文本行尾转换影响；构建、环境和冒烟文本通过本报告中的成功标记复核。
Git 跟踪源码、场景、测试、文档和验收证据；Windows Build 通过相同 Unity 版本和
上述构建入口重新生成。
