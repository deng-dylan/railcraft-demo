...
> **状态：冻结历史 Demo。** 本目录保留项目自有的 Godot `v0.1.0-demo` 源码、
> 测试和发布证据，用于复核与回溯。当前开发主线已切换到
> [`../railcraft-unity`](../railcraft-unity) 的 Unity 第三人称流程白盒。
...
- 本目录不接收当前第三人称玩法、58题题库、14零件或流程图装配功能。
- 历史场景、数据、自动化测试、发布说明和验收截图继续保留，避免破坏
  `v0.1.0-demo` 的复现能力。
- 必要的安全修复、工具链修复或可追溯勘误使用独立提交，并明确说明没有改变历史
  产品范围。新玩法统一在 Unity 主线实现。
- 外部 Godot 第三人称样例位于 `prototypes/`，与本项目自有历史 Demo 分别保存。
